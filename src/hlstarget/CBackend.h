...
private:
  ...
  raw_ostream& printFunctionProto(raw_ostream& Out, FunctionType* Ty, std::pair<AttributeSet, CallingConv::ID> Attrs,
                                  const std::string& Name, Function::ArgumentListType* ArgList);
  raw_ostream& printFunctionProto(raw_ostream& Out, Function* F)
  {
    return printFunctionProto(Out, F->getFunctionType(), std::make_pair(F->getAttributes(), F->getCallingConv()),
                              GetValueName(F), NULL);
  }
  ...
  void printIntrinsicDefinition(FunctionType* funT, unsigned Opcode, std::string OpName, raw_ostream& Out);
